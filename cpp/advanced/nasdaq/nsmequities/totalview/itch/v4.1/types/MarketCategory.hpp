#pragma once

#include <cstddef>
#include <algorithm>
#include <array>
#include <optional>
#include <string_view>
#include "../cache/Required.hpp"

namespace nasdaq::nsmequities::totalview::itch::v4_1 {

// market_category
struct market_category {

    enum class enum_type : char {
        nyse = 'N',
        amex = 'A',
        arca = 'P',
        nasdaq_gsm = 'Q',
        nasdaq_gm = 'G',
        nasdaq_cm = 'S',
        bats = 'Z'
    };

    static constexpr std::array<std::pair<std::string_view, enum_type>, 7> from_string_map = {{
        {"Amex", enum_type::amex},
        {"Arca", enum_type::arca},
        {"Bats", enum_type::bats},
        {"Nasdaq Cm", enum_type::nasdaq_cm},
        {"Nasdaq Gm", enum_type::nasdaq_gm},
        {"Nasdaq Gsm", enum_type::nasdaq_gsm},
        {"Nyse", enum_type::nyse}
    }};

    static constexpr std::string_view to_string(enum_type value) {
        switch (value) {
            case enum_type::nyse: return "Nyse";
            case enum_type::amex: return "Amex";
            case enum_type::arca: return "Arca";
            case enum_type::nasdaq_gsm: return "Nasdaq Gsm";
            case enum_type::nasdaq_gm: return "Nasdaq Gm";
            case enum_type::nasdaq_cm: return "Nasdaq Cm";
            case enum_type::bats: return "Bats";
            default: return "unknown";
        }
    }

    static constexpr std::optional<enum_type> from_string(std::string_view str) {
        auto it = std::lower_bound(
            from_string_map.begin(),
            from_string_map.end(),
            str,
            [](const auto& pair, std::string_view s) { return pair.first < s; }
        );
        if (it != from_string_map.end() && it->first == str) {
            return it->second;
        }
        return std::nullopt;
    }

    static constexpr const char* name = "market_category";
    static constexpr std::size_t size = 1;
    static constexpr bool is_optional = false;

    using result_type = required<market_category::enum_type>;
    using storage_type = result_type;

    constexpr market_category()
     : value{ enum_type::nyse } {}

    constexpr market_category(enum_type v)
     : value{ v } {}

    [[nodiscard]] constexpr result_type get() const {
        return result_type{value};
    }

    constexpr void set(enum_type v) {
        value = v;
    }

    constexpr void set(result_type value) {
        if (value.has_value())
            set(value.value());
        else
            set(enum_type::bats);
    }

  protected:
    enum_type value;
};
}
