...
inline std::ostream& operator<<(std::ostream& os, const udp_packet_header& value) {
    os << "udp_session=\"" << value.udp_session.get_trimmed().value() << '"'
       << ",udp_sequence_number=" << value.udp_sequence_number.get().value()
...
inline std::ostream& operator<<(std::ostream& os, const sequenced_data_packet& msg) {
    os << "sequenced_message_type=\"" << itch_totalview::sequenced_message_type::to_string(msg.fields.sequenced_message_type.get().value()) << '"'
       << ",tail=" << json::tail_to_json_string(msg)
       ;
    return os;
}
