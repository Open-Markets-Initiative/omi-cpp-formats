...
// seq_action on_transport_header(const udp_packet_header&, const packet::Frame&)  — called once per segment, returns process or skip
// on_message(const <message_type>&, std::uint64_t packet_receive_time, const udp_packet_header&)  — called per message (via dispatch)
...
void process_segment(Handler& handler, const std::byte* data, std::size_t length, std::uint64_t packet_receive_time, const packet::Frame& frame) {
    if (length < sizeof(udp_packet_header)) return;

    const auto* transport = udp_packet_header::parse(data);
    if (handler.on_transport_header(*transport, frame) != seq_action::process) return;

    auto count = transport->message_count.get().value();

    const std::byte* cursor = data + sizeof(udp_packet_header);
